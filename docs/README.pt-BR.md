...
- **Tags Padronizadas (DRY):** A infraestrutura adota a injeção centralizada de tags globais (`default_tags` no Provider) e mesclagem local (`merge(local.common_tags)`), garantindo estrita governança e controle de custos no _AWS Billing_ sem a necessidade de repetição manual de código.
